...
public class PlatformSpawner : MonoBehaviour 
{
    ...
    void Start() 
    {
        ...
        for(int i = 0; i < count; i++)
        {
            ...
            // Instantiate(GameObject original ,Vector3 position ,Quaternion rotation) : 게임 실행 중 게임 오브젝트 생성
            // Quaternion.identity : 회전값을 기본값으로 설정 (0, 0, 0)
            platforms[i] = Instantiate(platformPrefab, poolPosition, Quaternion.identity);
        } 
        // 마지막 배치 시점 초기화
        lastSpawnTime = 0f;
        // 다음번 배치까지의 간격을 0으로 초기화
        timeBetSpawn = 0f;
    }
    ...
    void Update() 
    {
        ...
        // Time.time : 게임 시작 후 시간이 몇초 지났는지 출력하는 변수 (현재 시점) 
        if(Time.time >= lastSpawnTime + timeBetSpawn)
        {
            // 기록된 마지막 배치 시점을 현재 시점으로 갱신
            lastSpawnTime = Time.time;

            // 다음 배치까지의 시간 간격을 timeBetSpawnMin, timeBetSpawnMax 사이에 랜덤 설정
            ...
            // 이때 발판의 Platform 컴포넌트의 OnEnable 매서드가 실행됨
